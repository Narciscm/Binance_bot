...
def evaluate_indicators(closes):
    ...
    if len(closes) == 0:
        ...
        return False
    rsi, short_ma, long_ma, macd, macd_signal, upper_band, lower_band, midlle_band = apply_indicators(closes)

    current_price = closes[-1]

    # Adăugăm mesaje de debug pentru a verifica valorile indicatorilor
    print(
        f"Evaluare pentru perechea analizată: Preț curent: {current_price}, RSI: {rsi[-1]:.4f}, MA scurt: {short_ma[-1]:.4f}, MA lung: {long_ma[-1]:.4f}, MACD: {macd[-1]:.4f}, MACD semnal: {macd_signal[-1]:.4f}")
